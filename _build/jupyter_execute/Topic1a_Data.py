...
# The use of the = sign in the world of computer code is very different from math. In math, = means to expressions are equal to each other. In the world of code, $X = Y$ should be understood NAME=VALUE or 'name (X) is assigned (=) a value (Y). For example, if I was to make a variable named X and give it a value 2 (for me to recall later) then I can write
...
# So far, this may seem simple enough. But, we rarely assign values and leave them there. Instead, the point of most computer codes involves updating the values of a variable. But suppose X is just the running tally of something I am counting (names that start wit 'S', friends with the same birthday,etc). I would then want to update the value when I found more instances. E.g. X is counting paragraphs in an application letter. I have X=2 now, but as I add more paragraphs, I might wan to add 1 to the value: 
...
# This might look peculiar, but we can check it did the right thing:
